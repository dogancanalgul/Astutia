...
#include <fstream>
#include <sstream>
#define HEIGHT 2//rand() % 4

World::World() : worldCenter(glm::vec3(0.f, 0.f, 0.f)), saveName("savegame.txt")
{
	srand(time(NULL));
	loadMap();
		...
		for (int i = 0; i < FIELDOFVIEW; ++i)
			for (int j = 0; j < FIELDOFVIEW; ++j) {
				...
				for (int k = 0; k < height; ++k)
					strBoxes.app('2');
					addChunk(i - FIELDOFVIEW / 2, j - FIELDOFVIEW / 2, strBoxes.str());
			}
	}
}
World::~World() {
...
void World::update(Player& p) {//Should work whenever player moves. But for now in every update
	glm::vec3 dif = p.cam.getPos() - worldCenter;
	dif.y = 0;
	if (glm::length(dif) >= 0.75f) {	
		if (dif.x >= 0.5f)
			movingRight();
		if (dif.x <= -0.5f)
			movingLeft();
